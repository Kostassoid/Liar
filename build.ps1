properties {
    ...
    $SolutionPath = "$BaseDir\src\Liar.sln"
...
task Pack -depends Test {
	...
	Exec { & ".\src\.nuget\nuget.exe" pack nuget\Liar.nuspec -version $Version -OutputDirectory $OutputPath }
}
...
task Build -depends Clean {
	...
	Copy-Item "$BaseDir\src\main\Liar\bin\$Configuration\*.*" $BuiltPath -Recurse
}
